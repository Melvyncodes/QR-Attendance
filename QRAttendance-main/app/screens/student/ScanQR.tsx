...
export default function ScanQR() {
  ...
  const [result, setResult] = useState<'success' | 'error' | 'already' | 'geo' | null>(null);
  ...
  const handleBarCodeScanned = async ({ data }: { data: string }) => {
    ...
    try {
      ...
      if (Date.now() > expires) {
        setResult('error');
        setMessage('This QR code has expired. Ask your lecturer to refresh it.');
        setProcessing(false);
        return;
      }
      ...
      const enrolledStudents: string[] = courseSnap.data().enrolledStudents || [];
      ...
      setResult('success');
      setMessage(`Attendance marked successfully for ${sessionData.courseTitle}!`);
    } catch {
      setResult('error');
      ...
      <View style={styles.container}>
        <View style={styles.content}>
          ...
          <View style={styles.center}>
            ...
            <Text style={styles.webSubtitle}>We need camera access to scan QR codes</Text>
  ...
  // ── Result / Camera ───────────────────────────────────────────────────────
  const resultConfig = {
    success: { icon: 'checkmark-circle', color: '#27AE60', title: 'Attendance Marked!', border: '#27AE60', btnLabel: 'Done', onPress: () => router.back() },
    already: { icon: 'information-circle', color: '#F39C12', title: 'Already Marked', border: '#F39C12', btnLabel: 'Go Back', onPress: () => router.back() },
    error:   { icon: 'close-circle', color: '#E74C3C', title: 'Error', border: '#E74C3C', btnLabel: 'Try Again', onPress: resetScanner },
    geo:     { icon: 'location-outline', color: '#E74C3C', title: 'Too Far Away', border: '#E74C3C', btnLabel: 'Try Again', onPress: resetScanner },
  } as const;
      ...
      <View style={styles.content}>
        ...
        {result ? (
          <View style={styles.resultContainer}>
            <View style={[styles.resultCard, { borderTopColor: resultConfig[result].border }]}>
              <Ionicons name={resultConfig[result].icon as any} size={64} color={resultConfig[result].color} />
              <Text style={styles.resultTitle}>{resultConfig[result].title}</Text>
              ...
              <TouchableOpacity style={styles.scanAgainButton} onPress={resultConfig[result].onPress}>
                <Text style={styles.scanAgainText}>{resultConfig[result].btnLabel}</Text>
              </TouchableOpacity>
            </View>
          </View>
        ) : (
            ...
            >
              <View style={styles.overlay}>
                ...
                <Text style={styles.scanText}>
                  {processing ? 'Checking location & processing…' : 'Point camera at QR code'}
                </Text>
                {processing && <ActivityIndicator size="large" color="#fff" style={{ marginTop: 16 }} />}
              </View>
            </CameraView>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  topBar: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', paddingHorizontal: 24, paddingVertical: 16, paddingTop: isWeb ? 22 : 50, borderBottomWidth: 1, borderBottomColor: '#E0E0E0', gap: 12, minHeight: isWeb ? 69 : 80, zIndex: 10 },
  ...
  center: { flex: 1, justifyContent: 'center', alignItems: 'center', gap: 16, padding: 24, backgroundColor: '#F5F6FA' },
  webTitle: { fontSize: 20, fontWeight: 'bold', color: '#2C3E7A', textAlign: 'center' },
  webSubtitle: { fontSize: 14, color: '#666', textAlign: 'center', paddingHorizontal: 40 },
  permissionButton: { backgroundColor: '#2C3E7A', paddingHorizontal: 24, paddingVertical: 12, borderRadius: 8, marginTop: 8 },
  ...
  scanText: { color: '#fff', fontSize: 16, marginTop: 24, fontWeight: '600', textAlign: 'center', paddingHorizontal: 20 },
  resultContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: 24, backgroundColor: '#F5F6FA' },
  resultCard: { backgroundColor: '#fff', borderRadius: 16, padding: 32, alignItems: 'center', gap: 16, width: '100%', maxWidth: 400, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 8, elevation: 4, borderTopWidth: 4 },
  resultTitle: { fontSize: 22, fontWeight: 'bold', color: '#2C3E7A' },
  resultMessage: { fontSize: 14, color: '#666', textAlign: 'center', lineHeight: 22 },
  geoBox: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#FDEDEC', borderRadius: 8, padding: 12, width: '100%' },
  geoBoxText: { fontSize: 13, color: '#E74C3C', flex: 1 },
  scanAgainButton: { backgroundColor: '#2C3E7A', paddingHorizontal: 32, paddingVertical: 12, borderRadius: 8, marginTop: 8 },
