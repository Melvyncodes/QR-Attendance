...
const EditField = ({ label, value, onChange, placeholder, keyboardType, editable = true }: {
  label: string; value: string; onChange: (v: string) => void;
  placeholder?: string; keyboardType?: any; editable?: boolean;
}) => (
  <View style={styles.editField}>
    <Text style={styles.editLabel}>{label}</Text>
    <TextInput style={[styles.editInput, !editable && styles.editInputDisabled]}
      value={value} onChangeText={onChange} placeholder={placeholder}
      placeholderTextColor="#999" editable={editable}
      keyboardType={keyboardType || 'default'} autoCorrect={false} autoCapitalize="words" />
  </View>
);
...
export default function AdminProfile() {
  ...
  const roleLabel = (userData?.role || 'admin').charAt(0).toUpperCase() + (userData?.role || 'admin').slice(1);
  const roleBadgeLabel = (userData?.role || 'ADMIN').toUpperCase();

  const handlePhotoUploaded = async (base64: string) => {
    await updateDoc(doc(db, 'users', userData?.uid), { profilePhoto: base64 });
    if (setUserData) setUserData((prev: any) => ({ ...prev, profilePhoto: base64 }));
  };

  const handlePhotoRemoved = async () => {
    await updateDoc(doc(db, 'users', userData?.uid), { profilePhoto: '' });
    if (setUserData) setUserData((prev: any) => ({ ...prev, profilePhoto: '' }));
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const updates = { firstName: firstName.trim(), lastName: lastName.trim(), phoneNumber: phoneNumber.trim(), department: department.trim(), college: college.trim() };
  ...
  const handleCancel = () => {
    ...
    setPhoneNumber(userData?.phoneNumber || ''); setDepartment(userData?.department || '');
    setCollege(userData?.college || ''); setEditing(false);
  };

  const avatarLetter = (userData?.firstName?.[0] || 'A').toUpperCase();

  return (
    <View style={styles.container}>
      {(isWeb || sidebarOpen) && (
        <Sidebar role="admin" userName={userData ? `${userData.firstName} ${userData.lastName}` : 'Admin'} activeRoute="/screens/admin/Profile" />
      )}
      <View style={styles.content}>
        <View style={[styles.topBar, { paddingTop: isWeb ? 18 : 50 }]}>
          {!isWeb && (
          ...
          <Text style={[styles.headerTitle, { fontSize: isWeb ? 18 : 15 }]}>My Profile</Text>
          {!editing && (
        ...
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={styles.profileContainer}>
            <View style={styles.avatarArea}>
              ...
              />
              <Text style={styles.avatarName}>{userData?.firstName} {userData?.lastName}</Text>
              <View style={styles.roleBadge}><Text style={styles.roleBadgeText}>{roleBadgeLabel}</Text></View>
            </View>
            ...
            {editing ? (
              <View style={styles.card}>
                ...
                <EditField label="Department" value={department} onChange={setDepartment} placeholder="Enter department" />
                <EditField label="College" value={college} onChange={setCollege} placeholder="Enter college" />
              ...
              <>
                ...
                {(userData?.department || userData?.college) && (
                  <View style={styles.card}>
      ...
      {sidebarOpen && !isWeb && <TouchableOpacity style={styles.overlay} onPress={() => setSidebarOpen(false)} />}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  editButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#2C3E7A', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8, gap: 6 },
  ...
  roleBadge: { backgroundColor: '#EEF2FF', paddingHorizontal: 12, paddingVertical: 4, borderRadius: 12 },
  roleBadgeText: { color: '#2C3E7A', fontSize: 12, fontWeight: '700' },
  card: { backgroundColor: '#fff', borderRadius: 16, padding: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 4, elevation: 2, gap: 4 },
  cardTitle: { fontSize: 14, fontWeight: '700', color: '#2C3E7A', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8, paddingBottom: 8, borderBottomWidth: 1, borderBottomColor: '#F0F0F0' },
  infoRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#F5F6FA' },
  infoLabel: { fontSize: 13, color: '#666', flex: 1 },
  infoValue: { fontSize: 13, fontWeight: '600', color: '#2D3436', flex: 2, textAlign: 'right' },
  editField: { gap: 6, marginBottom: 12 },
  editLabel: { fontSize: 13, fontWeight: '600', color: '#2C3E7A' },
  editInput: { backgroundColor: '#F5F6FA', borderRadius: 8, padding: 12, fontSize: 14, color: '#2D3436', borderWidth: 1, borderColor: '#E0E0E0' },
  ...
  saveButton: { flex: 2, paddingVertical: 12, borderRadius: 8, backgroundColor: '#2C3E7A', alignItems: 'center' },
